...
class AppForm(qt.QMainWindow):
    ...
    def on_draw(self):
        ...
        self.colour_table.setHorizontalHeaderLabels(
            ['Description', 'Value (2)', 'Value (10)'])
        self.colour_table.setItem(
            0, 0, qt.QTableWidgetItem('Le'))
        self.colour_table.setItem(
            0, 1, qt.QTableWidgetItem(str(self.results['Le2'])))
        self.colour_table.setItem(
            0, 2, qt.QTableWidgetItem(str(self.results['Le10'])))
        self.colour_table.setItem(
            1, 0, qt.QTableWidgetItem('Lv'))
        self.colour_table.setItem(
            1, 1, qt.QTableWidgetItem(str(self.results['Lv2'])))
        self.colour_table.setItem(
            1, 2, qt.QTableWidgetItem(str(self.results['Lv10'])))
        self.colour_table.setItem(
            2, 0, qt.QTableWidgetItem('X'))
        self.colour_table.setItem(
            2, 1, qt.QTableWidgetItem(str(self.results['X2'])))
        self.colour_table.setItem(
            2, 2, qt.QTableWidgetItem(str(self.results['X10'])))
        self.colour_table.setItem(
            3, 0, qt.QTableWidgetItem('Y'))
        self.colour_table.setItem(
            3, 1, qt.QTableWidgetItem(str(self.results['Y2'])))
        self.colour_table.setItem(
            3, 2, qt.QTableWidgetItem(str(self.results['Y10'])))
        self.colour_table.setItem(
            4, 0, qt.QTableWidgetItem('Z'))
        self.colour_table.setItem(
            4, 1, qt.QTableWidgetItem(str(self.results['Z2'])))
        self.colour_table.setItem(
            4, 2, qt.QTableWidgetItem(str(self.results['Z10'])))
        self.colour_table.setItem(
            5, 0, qt.QTableWidgetItem('x'))
        self.colour_table.setItem(
            5, 1, qt.QTableWidgetItem(str(self.results['x2'])))
        self.colour_table.setItem(
            5, 2, qt.QTableWidgetItem(str(self.results['x10'])))
        self.colour_table.setItem(
            6, 0, qt.QTableWidgetItem('y'))
        self.colour_table.setItem(
            6, 1, qt.QTableWidgetItem(str(self.results['y2'])))
        self.colour_table.setItem(
            6, 2, qt.QTableWidgetItem(str(self.results['y10'])))
        self.colour_table.setItem(
            7, 0, qt.QTableWidgetItem('u'))
        self.colour_table.setItem(
            7, 1, qt.QTableWidgetItem(str(self.results['u2'])))
        self.colour_table.setItem(
            7, 2, qt.QTableWidgetItem(str(self.results['u10'])))
        self.colour_table.setItem(
            8, 0, qt.QTableWidgetItem('v'))
        self.colour_table.setItem(
            8, 1, qt.QTableWidgetItem(str(self.results['v2'])))
        self.colour_table.setItem(
            8, 2, qt.QTableWidgetItem(str(self.results['v10'])))
        self.colour_table.setItem(
            9, 0, qt.QTableWidgetItem('T'))
        self.colour_table.setItem(
            9, 1, qt.QTableWidgetItem(str(self.results['T2'])))
        self.colour_table.setItem(
            9, 2, qt.QTableWidgetItem(str(self.results['T10'])))
        self.colour_table.setItem(
            10, 0, qt.QTableWidgetItem('Duv'))
        self.colour_table.setItem(
            10, 1, qt.QTableWidgetItem(str(self.results['Duv2'])))
        self.colour_table.setItem(
            10, 2, qt.QTableWidgetItem(str(self.results['Duv10'])))
...
<http://www.gnu.org/licenses/>."""
    ...
    def create_menu(self):
        ...
        save_spd_action = self.create_action(
            "&Save SPD table", shortcut="Ctrl+S",
            slot=self.on_save_spd, tip="Save the SPD table")
        save_colour_action = self.create_action(
            "Save colour data", slot=self.on_save_colour,
        ...
        self.add_actions(
            self.file_menu, (save_spd_action, save_colour_action,
                             None, quit_action))

        self.help_menu = self.menuBar().addMenu("&Help")
        about_action = self.create_action(
            "&About", shortcut='F1', slot=self.on_about,
    ...
    def create_main_frame(self):
        ...
        # Name of the port
        port_name = \
            '<enter port name, e.g., /dev/ttyS0 (posix), COM1 (windows)>'
        if os.name == 'posix':
            if os.path.exists('/dev/ttyUSB0'):  # linux usb-serial converter
                port_name = '/dev/ttyUSB0'
            elif os.path.exists('/dev/ttyS0'):  # linux serial port
                port_name = '/dev/ttyS0'
            elif os.path.exists('/dev/tty.usbserial'):  # mac usb-serial
                port_name = '/dev/tty.usbserial'        # converter
        elif os.name == 'nt':
            port_name = 'COM3'
