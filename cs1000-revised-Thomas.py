...
# -*- coding: utf-8 -*-
"""
cs1000: Library for communicating with the Minolta CS-1000

Copyright (C) 2012 Ivar Farup
...
class CS1000:
    ...
    """
    def __init__(self, port=0, baud_rate=19200):
        """
        Constructor: open serial connection.
        """
...
class AppForm(qt.QMainWindow):
    ...
    def on_save_spd(self):
        ...
        if path:
            ...
            np.savetxt(path, self.cs.results['spectrum'], '%.0f, %.4e')
    ...
    def on_save_colour(self):
        ...
        if path:
            ...
            f.write('Le, %.4f, %.4f\n' % (self.cs.results['Le2'], self.cs.results['Le10']))
            f.write('Lv, %.4f, %.4f\n' % (self.cs.results['Lv2'], self.cs.results['Lv10']))
            f.write('X, %.4f, %.4f\n' % (self.cs.results['X2'], self.cs.results['X10']))
            f.write('Y, %.4f, %.4f\n' % (self.cs.results['Y2'], self.cs.results['Y10']))
            f.write('Z, %.4f, %.4f\n' % (self.cs.results['Z2'], self.cs.results['Z10']))
            f.write('x, %.4f, %.4f\n' % (self.cs.results['x2'], self.cs.results['x10']))
            f.write('y, %.4f, %.4f\n' % (self.cs.results['y2'], self.cs.results['y10']))
            f.write('u, %.4f, %.4f\n' % (self.cs.results['u2'], self.cs.results['u10']))
            f.write('v, %.4f, %.4f\n' % (self.cs.results['v2'], self.cs.results['v10']))
            f.write('T, %s, %s\n' % (self.cs.results['T2'], self.cs.results['T10']))
            f.write('Duv, %s, %s\n' % (self.cs.results['Duv2'], self.cs.results['Duv10']))
            f.close()
